...
--     search_dirs = { '.' }, -- Search in current directory
--     use_regex = true,
--   }, {
...
--         vim.cmd(string.format("silent! grep! '%s' %s", selection.value, selection.path))
--         vim.cmd 'copen'
--       end)
--       return true
--     end,
...
-- vim.api.nvim_set_keymap('n', '<leader>wr', ':lua find_and_replace()<CR>', { noremap = true, silent = true })
--
--
-- Generated by Sourcegraph claude. I am not smart enough to write this yet. I
-- don't understand nvim apis and lua this good.
function _G.telescope_find_replace_in_buffer()
  ...
  local initial_search_term = vim.fn.mode() == 'v' and get_visual_selection() or get_current_word()

  pickers
    .new({}, {
      prompt_title = 'Find and Replace in Buffer',
      finder = finders.new_oneshot_job(
        { 'rg', '--line-number', '--column', '--with-filename', '--no-heading', '--color', 'never', initial_search_term, vim.fn.expand '%:p' },
        {
          entry_maker = function(entry)
            local _, line, col, text = entry:match '([^:]+):(%d+):(%d+):(.*)'
            return {
              value = text,
              display = text,
              ordinal = text,
        ...
        actions.select_default:replace(function()
          ...
          if search_term ~= '' then
            ...
            if replace_term ~= '' then
              local cmd = string.format('%%s/\\v%s/%s/gc', vim.fn.escape(search_term, '/\\'), vim.fn.escape(replace_term, '/\\'))
              vim.cmd(cmd)
            end
          else
...
local function add_license_header()
  local project_path = vim.fn.expand '%:p:h'
  local bunnynote_path = '/Users/swaingotnochill/sources/hackathons/appwrite_hacktoberfest/client/bunnynote'

  if string.sub(project_path, 1, #bunnynote_path) == bunnynote_path then
...
-- Autocommand to call the function when creating a new file.
vim.api.nvim_create_autocmd({ 'BufNewFile' }, {
  pattern = '*',
  callback = add_license_header,
})
